...
void pushEvent(GameState& s,std::string kind,real x=0,real y=0,int color=0){
...
real healingScale(const GameState& s){return hasCurse(s,"famine")?.5:1.0;}
real enemyXp(const std::string& type){const auto& e=enemyDefs().at(type);return e.xp>0?e.xp:std::max(1.0,std::round(e.hp/14.0));}

Player* nearestPlayer(GameState& s, const Enemy& e){Player* best=nullptr;real bd=std::numeric_limits<real>::max();for(auto& [_,p]:s.players)if(p.alive){real d=distanceSq({e.x,e.y},{p.x,p.y});if(d<bd){bd=d;best=&p;}}return best;}
Enemy* nearestEnemy(GameState& s,Vec2 at,real range){Enemy* best=nullptr;real bd=range*range;for(auto& e:s.enemies)if(e.hp>0){real d=distanceSq(at,{e.x,e.y});if(d<bd){bd=d;best=&e;}}return best;}
...
void grantXp(GameState& s,Player& p,real amount,Random& random){
...
bool hurt(GameState& s,Player& p,real damage){
  if(!p.alive||p.hitCooldown>0||!p.pendingPowers.empty()||p.invulnerableFor>0)return false;
  if(hasCurse(s,"brittle"))damage*=1.25;if(s.bloodPact)damage*=1.3;real taken=std::max(2.0,damage-p.armor);
...
void addDrop(GameState& s,real x,real y,const std::string& type,real value){
  if(type=="gem" && (s.gems.size()>=MAX_DROPS||s.gems.size()>=140)){
    Drop* target=nullptr;real best=s.gems.size()>=MAX_DROPS?std::numeric_limits<real>::max():90.0*90.0;
    for(auto& g:s.gems)if(g.type=="gem"&&!g.dead){real d=distanceSq({g.x,g.y},{x,y});if(d<best){best=d;target=&g;}}
...
Enemy* spawnEnemy(GameState& s,const std::string& type,real x,real y,real hpScale=1,bool elite=false,bool minion=false,bool thief=false){
  if(s.enemies.size()>=MAX_ENEMIES)return nullptr;auto it=enemyDefs().find(type);if(it==enemyDefs().end())return nullptr;real hp=it->second.hp*hpScale*(elite?7:1);
...
void dropLoot(GameState& s,Enemy& e,Random& rnd){
  ...
  real roll=rnd.next();if(roll<.05)addDrop(s,e.x,e.y,"heart",25);else if(roll<.25)addDrop(s,e.x,e.y,"greenGem",25);else if(roll<.28)addDrop(s,e.x,e.y,"coin",1);
}
...
void onKill(GameState& s,Enemy& e,Player* source,Random& rnd,bool shard){
  ...
  if(def.behavior=="splitter"){real scale=e.maxHp/def.hp/(e.elite?7:1);for(int n=0;n<2;n++){real a=rnd.next()*PI*2;spawnEnemy(s,"slimelet",e.x+std::cos(a)*18,e.y+std::sin(a)*18,scale,false,true);}}
  if(source&&!shard&&e.slowFor>0&&rankOf(*source,"shatter")){
    int count=rankOf(*source,"avalanche")?8:4;real mult=rankOf(*source,"avalanche")?.9:.6;real off=rnd.next()*PI;
    for(int n=0;n<count&&s.shots.size()<MAX_SHOTS;n++){real a=off+n*PI*2/count;Shot sh;sh.x=e.x;sh.y=e.y;sh.vx=std::cos(a)*420;sh.vy=std::sin(a)*420;sh.ttl=.5;sh.color=0;sh.damage=source->damage*mult;sh.pierce=1;sh.hitIds={e.id};sh.owner=source->id;sh.shard=true;s.shots.push_back(std::move(sh));}
  }
  if(e.thief)thiefDown(s,e);if(e.elite)pushEvent(s,"eliteDown",e.x,e.y);
}

void track(Player* p,const std::string& kind,real amount){if(!p||amount<=0)return;p->stats.damage+=amount;p->stats.by[kind]+=amount;}

bool damageEnemy(GameState& s,Enemy& e,real damage,Random& rnd,Player* source=nullptr,bool slow=false,bool shard=false,const std::string& element="",const std::string& kind="spell"){
  ...
  real bonus=0;if(!reaction.empty()&&e.comboAt<=s.time){e.comboAt=s.time+1;std::string setter=reaction=="thermal"?e.slowBy:reaction=="conduction"?e.rootBy:e.burnBy;bool team=source&&!setter.empty()&&setter!=source->id&&s.players.contains(setter);bonus=(source?source->damage:damage)*(team?2:1.5);if(team){for(Player* p:{source,&s.players.at(setter)})p->specialCharge=std::min(100.0,p->specialCharge+6);}if(reaction=="thermal"){e.slowFor=0;e.freezeFor=0;}{Event ev;ev.kind="combo";ev.x=e.x;ev.y=e.y;ev.color=source?source->color:0;ev.variant=team?1:0;ev.text=reaction;pushEvent(s,std::move(ev));}}
  if(element=="fire"){e.burningFor=1.5;if(source)e.burnBy=source->id;}real total=damage+bonus,dealt=std::min(e.hp,total);if(total>0){track(source,kind,dealt*damage/total);track(source,"combo",dealt*bonus/total);}e.hp-=total;
  if(slow){e.slowFor=1.2;if(source)e.slowBy=source->id;}if(e.hp<=0){onKill(s,e,source,rnd,shard);return true;}return false;
}

Vec2 dashDirection(const Player& p,Vec2 in){real l=std::hypot(in.x,in.y);if(l>.01)return {in.x/l,in.y/l};l=std::hypot(p.moveX,p.moveY);return l?Vec2{p.moveX/l,p.moveY/l}:Vec2{0,1};}
...
real SeededRandom::next(){a_+=0x6d2b79f5u;std::uint32_t t=a_;t=(t^(t>>15))*(t|1u);t^=t+(t^(t>>7))*(t|61u);return static_cast<real>(t^(t>>14))/4294967296.0;}
...
std::vector<std::string> availablePowers(const Player& p,Random& rnd,bool coop,const std::vector<std::string>& exclude){
 std::vector<std::string> pool;for(const auto& [id,_]:powerDefs())if(std::find(exclude.begin(),exclude.end(),id)==exclude.end()&&eligible(p,id,coop))pool.push_back(id);std::vector<std::string> out;auto w=[](const std::string& k){return k=="evolution"?1000.0:k=="signature"?1.6:k=="weapon"?1.25:k=="coop"?.8:1.0;};while(!pool.empty()&&out.size()<3){real total=0;for(auto& id:pool)total+=w(powerDefs().at(id).kind);real roll=rnd.next()*total;std::size_t i=0;for(;i<pool.size();i++){roll-=w(powerDefs().at(pool[i]).kind);if(roll<0)break;}if(i>=pool.size())i=pool.size()-1;out.push_back(pool[i]);pool.erase(pool.begin()+i);}return out;
}
...
real phaseDuration(const GameState& s){return campaignOf(s).seconds;}real phaseClock(const GameState& s){return s.phaseTime*300.0/phaseDuration(s);}real curseReward(const GameState& s){real r=1;for(auto& c:s.curses)r+=c=="swarm"?.25:c=="frenzy"?.2:c=="brittle"?.25:c=="famine"?.15:c=="tyrant"?.25:c=="nobility"?.2:0;return r;}
Difficulty difficultyAt(real clock,int playerCount,int phase,const GameState* s){real minutes=phase*3+clock/60,extra=std::max(0,playerCount-1),loop=s?s->loop:0;int count=1+static_cast<int>(std::floor(minutes/1.5))+static_cast<int>(std::floor(extra/2.0))+loop;Difficulty d;d.spawnInterval=std::max(.25,.5-minutes*.04);d.hpScale=(1+minutes*.215+minutes*minutes*.018)*(1+extra*.3)*(1+loop*.6);d.damageScale=std::min(2.15,1+minutes*.091)*(1+loop*.15);d.speedScale=std::min(1.22,1+minutes*.028)*(s&&hasCurse(*s,"frenzy")?1.15:1);d.spawnCount=(s&&hasCurse(*s,"swarm"))?static_cast<int>(std::ceil(count*1.35)):count;return d;}
...
void converge(GameState& s,Player& p,Random& rnd){
  ...
  if(it==s.recentSpecials.end()){s.recentSpecials.push_back({p.id,s.time,p.x,p.y});return;}auto pit=s.players.find(it->id);if(pit==s.players.end())return;auto& q=pit->second;real x=(p.x+q.x)/2,y=(p.y+q.y)/2,damage=(p.damage+q.damage)*4;s.recentSpecials.erase(it);for(auto& e:s.enemies)if(e.hp>0&&distanceSq({x,y},{e.x,e.y})<320.0*320.0)damageEnemy(s,e,damage,rnd,&p,false,false,"","convergence");pushEvent(s,"convergence",x,y,p.color);
}
...
real hitRadius(const Enemy& e,const Spell& sp){if(e.boss)return enemyDefs().at(e.type).radius+sp.radius-29;return e.elite?sp.radius+8:sp.radius;}

void updateShots(GameState& s,real dt,Random& rnd,EnemyGrid& enemyGrid){
  for(auto& sh:s.shots){
    ...
    if(sh.boomerang&&owner&&owner->alive){
      ...
      if(sh.returning){
        real a=std::atan2(owner->y-sh.y,owner->x-sh.x);
    ...
    const real reach=(sh.fullmoon&&sh.returning)?1.35:1;
    while(sh.ttl>0 && sh.pierce>0){
      ...
      enemyGrid.query(static_cast<float>(sh.x),static_cast<float>(sh.y),115.0f,[&](Enemy& e,float d2){
        ...
        const real r=hitRadius(e,sp)*reach;
      ...
      if(owner&&sh.color==2&&rankOf(*owner,"ricochet")){
        Enemy* target=nullptr;real bd=220.0*220.0;
        ...
        if(target){real speed=std::hypot(sh.vx,sh.vy),a=std::atan2(target->y-sh.y,target->x-sh.x);sh.vx=std::cos(a)*speed;sh.vy=std::sin(a)*speed;}
        break;
      }
    }
  }
...
void updateOrbit(GameState& s,Player& p,int rank,real dt,Random& rnd){bool evolved=rankOf(p,"constellation"),solar=rankOf(p,"solarcrown");static const int counts[5]={1,2,2,3,3};int count=counts[rank-1]+(evolved?2:0);real radius=78+rank*4+(evolved?20:0),damage=p.damage*(.55+.1*rank)*(evolved?2:1)*(solar?1.3:1);p.orbitAngle=std::fmod(p.orbitAngle+2.7*dt,PI*2);for(int n=0;n<count;n++){real a=p.orbitAngle+n*PI*2/count,x=p.x+std::cos(a)*radius,y=p.y+std::sin(a)*radius,r=16+(evolved?26:18);for(auto& e:s.enemies)if(e.hp>0&&distanceSq({x,y},{e.x,e.y})<r*r&&hitOnce(e,p.color,s.time,.5))damageEnemy(s,e,damage,rnd,&p,false,false,solar?"fire":"","orbit");}}
void updateAura(GameState& s,Player& p,int rank,real dt,Random& rnd,const PlayerList& alive){p.auraTimer-=dt;if(p.auraTimer>0)return;p.auraTimer=.5;bool evolved=rankOf(p,"sanctuary");real radius=70+rank*10+(evolved?30:0),damage=p.damage*(.25+.08*rank);for(auto& e:s.enemies)if(e.hp>0&&distanceSq({p.x,p.y},{e.x,e.y})<radius*radius)damageEnemy(s,e,damage,rnd,&p,evolved&&!e.boss,false,"","aura");if(evolved)for(auto* a:alive)if(distanceSq({a->x,a->y},{p.x,p.y})<radius*radius&&a->sanctuaryAt<=s.time){a->hp=std::min(a->maxHp,a->hp+2);a->sanctuaryAt=s.time+.5;}}
void updateChain(GameState& s,Player& p,int rank,real dt,Random& rnd){
  ...
  for(int n=0;n<=jumps&&target;n++){
    ...
    target=nullptr;real bd=150.0*150.0;
    for(auto& e:s.enemies)if(e.hp>0&&std::find(hit.begin(),hit.end(),e.id)==hit.end()){
      real d=distanceSq(from,{e.x,e.y});if(d<bd){bd=d;target=&e;}
    }
  }
  pushEvent(s,std::move(ev));
}

void updateRunes(GameState& s,Player& p,int rank,real dt){p.runeTimer-=dt;if(p.runeTimer>0)return;p.runeTimer=3.2-.25*rank;bool evolved=rankOf(p,"minefield");int count=evolved?3:1;real radius=70+8*rank+(evolved?30:0);for(int n=0;n<count&&s.runes.size()<MAX_RUNES;n++){real a=n*PI*2/count+s.time,off=count>1?60:0;s.runes.push_back({nextId(s),p.x+std::cos(a)*off,p.y+std::sin(a)*off,radius,p.damage*(1.6+.3*rank),8,.4,p.id,p.color});}}
void updateFamiliar(GameState& s,Player& p,int rank,real dt,Random& rnd){
  real a=s.time*1.4+p.color;Vec2 home{p.x+std::cos(a)*62,p.y+std::sin(a)*62*.55-26};
  ...
  real follow=1-std::exp(-6*dt);pet.x+=(home.x-pet.x)*follow;pet.y+=(home.y-pet.y)*follow;pet.timer-=dt;if(pet.timer>0)return;
  bool evo=rankOf(p,"covenant");static const int targets[5]={1,1,2,2,3};
  const int wanted=std::min(5,targets[rank-1]+(evo?2:0));
  std::array<std::pair<real,Enemy*>,5> found{};int foundCount=0;
  for(auto& e:s.enemies)if(e.hp>0){
    const real d=distanceSq({pet.x,pet.y},{e.x,e.y});if(d>=380.0*380.0)continue;
  ...
  real damage=p.damage*(.9+.22*rank)*(evo?1.4:1);Event ev;ev.kind="familiar";ev.x=pet.x;ev.y=pet.y;ev.color=p.color;
...
void updateWeapons(GameState& s,real dt,Random& rnd,const PlayerList& alive){
  ...
  for(auto& z:s.zones){z.ttl-=dt;Player* owner=nullptr;auto oi=s.players.find(z.owner);if(oi!=s.players.end())owner=&oi->second;if(z.follow){if(!owner||!owner->alive){z.ttl=0;continue;}z.x=owner->x;z.y=owner->y;retain(s.enemyShots,[&](const EnemyShot& sh){return distanceSq({sh.x,sh.y},{z.x,z.y})>z.radius*z.radius;});}if(z.kind=="meteor"&&z.warning>0){z.warning-=dt;if(z.warning>0)continue;for(auto& e:s.enemies)if(e.hp>0&&distanceSq({z.x,z.y},{e.x,e.y})<z.radius*z.radius)damageEnemy(s,e,z.damage,rnd,owner,false,false,"fire","special");{Event ev;ev.kind="boom";ev.x=z.x;ev.y=z.y;ev.r=z.radius;ev.color=1;pushEvent(s,std::move(ev));}}std::string kind=(z.kind!="burn"&&!z.kind.empty())?"special":"burn",elem=(z.kind=="burn"||z.kind=="meteor"||z.kind=="flameshield")?"fire":z.kind=="vortex"?"moon":"";for(auto& e:s.enemies)if(e.hp>0){real d=distanceSq({z.x,z.y},{e.x,e.y});if(d<z.radius*z.radius){if(z.kind=="roots"){e.rootFor=.5;if(owner)e.rootBy=owner->id;}if(z.pull>0&&!e.boss&&d>400){real dist=std::sqrt(d),step=std::min(dist-20,z.pull*dt);e.x+=(z.x-e.x)/dist*step;e.y+=(z.y-e.y)/dist*step;}damageEnemy(s,e,z.dps*dt,rnd,owner,z.slow,false,elem,kind);}}if(z.kind=="vortex"&&z.ttl<=0){for(auto& e:s.enemies)if(e.hp>0&&distanceSq({z.x,z.y},{e.x,e.y})<z.radius*z.radius)damageEnemy(s,e,z.damage,rnd,owner,false,false,"moon","special");{Event ev;ev.kind="boom";ev.x=z.x;ev.y=z.y;ev.r=z.radius;ev.color=3;pushEvent(s,std::move(ev));}}}
...
std::string pickType(int phase,real clock,Random& rnd){real roll=rnd.next(),cursor=0;struct S{const char*t;real after,w;};native::StaticVector<S,2> specials;switch(phase){case 0:specials={{"slime",60,.18}};break;case 1:specials={{"eye",45,.16}};break;case 2:specials={{"bat",45,.16},{"brute",150,.08}};break;case 3:specials={{"slime",45,.18},{"scorpion",150,.08}};break;case 4:specials={{"revenant",60,.16},{"brute",150,.08}};break;default:specials={{"seer",45,.18},{"revenant",150,.1}};}
 for(auto& sp:specials){if(clock<sp.after)continue;cursor+=sp.w;if(roll<cursor)return sp.t;}real core=(roll-cursor)/(1-cursor);return phases()[phase].enemies[core<.6?0:1];}
Enemy* spawnAround(GameState& s,const Difficulty& d,Player& focus,real angle,const std::string& type,Random& rnd,real distance=-1,bool elite=false){real dist=distance>=0?distance:520+rnd.next()*120;return spawnEnemy(s,type,focus.x+std::cos(angle)*dist,focus.y+std::sin(angle)*dist,d.hpScale,elite);}
void runSchedule(GameState& s,const Difficulty& d,const PlayerList& alive,Random& rnd){static const std::array<std::pair<real,const char*>,6> beats={{{6,"opening"},{90,"elite"},{135,"ring"},{180,"elite"},{230,"ring"},{270,"elite"}}};while(s.scheduleCursor<beats.size()&&phaseClock(s)>=beats[s.scheduleCursor].first){auto [at,kind]=beats[s.scheduleCursor++];if(phaseClock(s)-at>2)continue;if(std::string(kind)=="opening"){for(auto* p:alive)for(int n=0;n<8;n++)spawnAround(s,d,*p,n*PI/4+rnd.next()*.3,phases()[s.phase].enemies[0],rnd);}else if(std::string(kind)=="elite"){int count=hasCurse(s,"nobility")?2:1;for(int n=0;n<count;n++){auto* p=alive[(std::size_t)(rnd.next()*alive.size())%alive.size()];auto type=phases()[s.phase].enemies[(std::size_t)(rnd.next()*2)%2];if(auto* e=spawnAround(s,d,*p,rnd.next()*PI*2,type,rnd,480,true))pushEvent(s,"elite",e->x,e->y);}}else{auto* p=alive[(std::size_t)(rnd.next()*alive.size())%alive.size()];int count=std::min<int>(16+(int)alive.size()*4,MAX_ENEMIES-(int)s.enemies.size());for(int n=0;n<count;n++)spawnAround(s,d,*p,n*PI*2/count,phases()[s.phase].enemies[1],rnd,440);if(count)pushEvent(s,"ring",p->x,p->y);}}}
void spawnHorde(GameState& s,real dt,const Difficulty& d,const PlayerList& alive,Random& rnd){runSchedule(s,d,alive,rnd);s.spawn-=dt;real crowd=hasCurse(s,"swarm")?1.2:1;int limit=std::min(MAX_ENEMIES,(int)std::floor((45+std::floor(phaseClock(s)*.3)+s.phase*20+alive.size()*18)*crowd));if(s.spawn>0||(int)s.enemies.size()>=limit)return;s.spawn=d.spawnInterval;int batch=std::min({d.spawnCount,limit-(int)s.enemies.size(),6});real off=rnd.next()*PI*2;for(int n=0;n<batch;n++){auto* p=alive[(s.spawnCursor+n)%alive.size()];real a=off+n*(PI*2/batch)+rnd.next()*.25;spawnAround(s,d,*p,a,pickType(s.phase,phaseClock(s),rnd),rnd);}s.spawnCursor=(s.spawnCursor+batch)%alive.size();}

void addHazard(GameState& s,Hazard h){if(s.hazards.size()<MAX_HAZARDS){h.warn0=h.warning;s.hazards.push_back(std::move(h));}}
void fireBoss(GameState& s,const Enemy& e,real a,const std::string& sprite,real speed){if(s.enemyShots.size()>=MAX_ENEMY_SHOTS)return;auto& d=enemyDefs().at(e.type);s.enemyShots.push_back({e.x,e.y,std::cos(a)*speed,std::sin(a)*speed,4,d.damage*.5,10,sprite});}
int stageOf(const Enemy& e){real r=e.hp/e.maxHp;return r>.66?1:r>.33?2:3;}
void enterStage(GameState& s,Enemy& e,int stage,const Difficulty& diff,const PlayerList& alive,Random& rnd){e.stage=stage;auto& def=enemyDefs().at(e.type);Hazard h;h.x=e.x;h.y=e.y;h.radius=210;h.warning=1;h.ttl=1.35;h.damage=def.damage+12;addHazard(s,h);int count=4+(int)alive.size()*2;for(int n=0;n<count;n++){real a=n*PI*2/count+rnd.next()*.2;spawnEnemy(s,phases()[s.phase].enemies[0],e.x+std::cos(a)*240,e.y+std::sin(a)*240,1+s.phase*.8,false,true);}e.attackCooldown=std::min(e.attackCooldown,2.0);Event ev;ev.kind="stage";ev.x=e.x;ev.y=e.y;ev.stage=stage;pushEvent(s,std::move(ev));}
void areaAttack(GameState& s,Enemy& e,Player& target,const PlayerList& alive,Random& rnd){real damage=enemyDefs().at(e.type).damage+8;auto hazard=[&](real x,real y,real r,real warning=1.3){Hazard h;h.x=x;h.y=y;h.radius=r;h.damage=damage;h.warning=warning;h.ttl=warning+.35;addHazard(s,h);};int stage=e.stage;if(e.type=="treant"){hazard(e.x,e.y,175);if(stage>=2){real a=std::atan2(target.y-e.y,target.x-e.x);for(int n=1;n<=4;n++)hazard(e.x+std::cos(a)*(140+n*110),e.y+std::sin(a)*(140+n*110),70,1+n*.15);}}
 ...
 else if(e.type=="bogwarden"){int count=stage==3?8:6;for(int n=0;n<count;n++){real a=n*PI*2/count;hazard(target.x+std::cos(a)*170,target.y+std::sin(a)*170,62,1.4+n*.08);}if(stage>=2)hazard(target.x,target.y,75,1.8);}
 ...
 else if(e.type=="umbra"){real a=std::atan2(target.y-e.y,target.x-e.x);for(int line=0;line<(stage>=2?2:1);line++){real dir=a+line*PI/2;for(int n=-2;n<=2;n++){if(line&&n==0)continue;hazard(target.x+std::cos(dir)*n*120,target.y+std::sin(dir)*n*120,58,1.1+(n+2)*.18);}}}
...
void rangedAttack(GameState& s,Enemy& e,Player& target){int stage=e.stage;real angle=std::atan2(target.y-e.y,target.x-e.x);std::string sprite=(e.type=="treant"||e.type=="bogwarden")?"thorn":(e.type=="lich"||e.type=="archon")?"bolt":e.type=="umbra"?"blade":"fire";real speed=(e.type=="lich"||e.type=="archon")?230:200;auto radial=[&](int c,real o=0){for(int n=0;n<c;n++)fireBoss(s,e,o+n*PI*2/c,sprite,speed*.85);};auto fan=[&](int c){for(int n=0;n<c;n++)fireBoss(s,e,angle+(n-(c-1)/2.0)*.23,sprite,speed);};if(e.type=="bogwarden"){fan(3+stage*2);if(stage==3)radial(8,s.time);}else if(e.type=="archon"){radial(4+stage*4,s.time*.45);if(stage==3)fan(3);}else if(e.type=="umbra"){radial(6+stage*4,s.time*.6);if(stage>=2)fan(5);}else if(stage==1)fan(e.type=="demon"?5:3);else if(e.type=="lich")radial(stage==2?12:16,s.time);else if(e.type=="treant"){fan(5);if(stage==3)radial(10,s.time);}else fan(7);}
void bossBrain(GameState& s,Enemy& e,Player& target,real dt,const Difficulty& diff,const PlayerList& alive,Random& rnd){int st=stageOf(e);if(st>e.stage)enterStage(s,e,st,diff,alive,rnd);static const real pace[3]={1,.8,.65};static const std::unordered_map<std::string,real> area={{"treant",4.5},{"lich",4.5},{"demon",3.6},{"bogwarden",4.8},{"archon",4.5},{"umbra",4.6}},range={{"treant",3.6},{"lich",3.2},{"demon",2.6},{"bogwarden",3.4},{"archon",3.6},{"umbra",3.2}};e.attackCooldown-=dt;if(e.attackCooldown<=0){e.attackCooldown=area.at(e.type)*pace[e.stage-1];areaAttack(s,e,target,alive,rnd);}e.rangedCooldown-=dt;if(e.rangedCooldown<=0){e.rangedCooldown=range.at(e.type)*pace[e.stage-1];rangedAttack(s,e,target);}if((e.type=="demon"&&e.stage>=2)||(e.type=="umbra"&&e.stage==3)){e.dashTimer-=dt;if(e.dashWarn>0){e.dashWarn-=dt;if(e.dashWarn<=0){e.dash=.5;e.dashSpeed=560;}}else if(e.dash>0)e.dash-=dt;else if(e.dashTimer<=0){e.dashTimer=6;e.dashWarn=.8;e.dashAngle=std::atan2(target.y-e.y,target.x-e.x);pushEvent(s,"dash",e.x,e.y);}}}

void behaveEnemy(GameState& s,Enemy& e,Player& target,real& angle,real& speed,real dt,const Difficulty& diff,Random& rnd,bool& move){const auto& def=enemyDefs().at(e.type);real d2=distanceSq({e.x,e.y},{target.x,target.y});if(def.behavior=="charger"){e.chargeTimer=(e.chargeTimer>0?e.chargeTimer:3.4*(.5+(e.id%7)/7.0))-dt;if(e.windup>0){e.windup-=dt;if(e.windup<=0)e.dash=.45;move=false;return;}if(e.dash>0){e.dash-=dt;angle=e.dashAngle;speed*=3.2;return;}if(e.chargeTimer<=0&&d2<320.0*320.0){e.chargeTimer=3.4;e.windup=.5;e.dashAngle=angle;move=false;return;}}
 ...
 else if(def.behavior=="bomber"){if(e.fuse>0){e.fuse-=dt;if(e.fuse<=0){e.hp=0;e.exploded=true;real damage=def.damage*2.2*diff.damageScale;for(auto& [_,p]:s.players)if(p.alive&&distanceSq({e.x,e.y},{p.x,p.y})<70.0*70.0)hurt(s,p,damage);pushEvent(s,"boom",e.x,e.y);}move=false;return;}if(d2<60.0*60.0){e.fuse=.6;move=false;return;}}}

void updateEnemies(GameState& s,real dt,const Difficulty& diff,const PlayerList& alive,Random& rnd,EnemyGrid& separationGrid){++s.tick;for(auto& e:s.enemies){if(e.hp<=0)continue;e.age+=dt;e.burningFor=std::max(0.0,e.burningFor-dt);e.rootFor=std::max(0.0,e.rootFor-dt);e.freezeFor=std::max(0.0,e.freezeFor-dt);if(!e.boss&&e.freezeFor>0)continue;Player* target=nullptr;real bd=std::numeric_limits<real>::max();for(auto* p:alive){real d=distanceSq({e.x,e.y},{p->x,p->y});if(d<bd){bd=d;target=p;}}if(!target)continue;const auto& def=enemyDefs().at(e.type);real angle=std::atan2(target->y-e.y,target->x-e.x);e.slowFor=std::max(0.0,e.slowFor-dt);e.touchCooldown=std::max(0.0,e.touchCooldown-dt);real slow=e.slowFor>0?(e.boss?.85:.6):1,speed=def.speed*slow*diff.speedScale*(e.elite?.9:1);if(e.rootFor>0)speed*=e.boss?.75:0;bool move=true;if(e.boss){bossBrain(s,e,*target,dt,diff,alive,rnd);if(e.dash>0)speed=e.dashSpeed;real heading=e.dash>0?e.dashAngle:angle;if(e.dashWarn<=0){e.x+=std::cos(heading)*speed*dt;e.y+=std::sin(heading)*speed*dt;}}
 else if(e.thief){real flee=angle+PI+std::sin(e.age*2.3)*.6,pace=def.speed*1.45*slow*(e.rootFor>0?0:1);e.x+=std::cos(flee)*pace*dt;e.y+=std::sin(flee)*pace*dt;continue;}else{behaveEnemy(s,e,*target,angle,speed,dt,diff,rnd,move);if(move){e.x+=std::cos(angle)*speed*dt;e.y+=std::sin(angle)*speed*dt;}}
 if(e.hp<=0)continue;real reach=e.boss?def.radius+15:e.elite?44:34;if(e.touchCooldown<=0&&distanceSq({e.x,e.y},{target->x,target->y})<reach*reach){real mult=(e.elite?1.5:1)*(e.dash>0?1.5:1);if(hurt(s,*target,def.damage*diff.damageScale*.73*mult))e.touchCooldown=.8;}}
...
void updateHazards(GameState& s,real dt,const PlayerList& alive){for(auto& h:s.hazards){h.warning-=dt;h.ttl-=dt;if(h.warning<=0&&!h.fired){h.fired=true;for(auto* p:alive)if(distanceSq({h.x,h.y},{p->x,p->y})<h.radius*h.radius)hurt(s,*p,h.damage);}}retain(s.hazards,[](const Hazard&h){return h.ttl>0;});}

void collectOne(GameState& s,Drop& g,Player& p,Random& rnd){g.dead=true;if(g.type=="heart")p.hp=std::min(p.maxHp,p.hp+g.value*healingScale(s));else if(g.type=="greenGem")p.specialCharge=std::min(100.0,p.specialCharge+g.value);else if(g.type=="coin"){for(auto& [_,a]:s.players){real earned=g.value*campaignOf(s).coins*a.coinMult+a.coinFrac;a.coins+=(int)std::floor(earned);a.coinFrac=earned-std::floor(earned);}}else if(g.type=="magnet"){for(auto& o:s.gems)if(o.type=="gem"){o.pull=p.id;o.ttl=std::max(o.ttl,10.0);}pushEvent(s,"magnet",p.x,p.y);}else if(g.type=="chest"){for(auto& [_,a]:s.players){++a.pendingChests;a.coins+=5;}pushEvent(s,"chest",p.x,p.y);}else{real reward=g.value*campaignOf(s).xp;for(auto& [_,a]:s.players)grantXp(s,a,reward,rnd);}}
void collectDrops(GameState& s,real dt,const PlayerList& survivors,Random& rnd){for(auto& g:s.gems){if(g.pull.empty())g.ttl-=dt;if(g.ttl<=0||survivors.empty())continue;Player* target=nullptr;if(!g.pull.empty()){auto it=s.players.find(g.pull);if(it!=s.players.end()&&it->second.alive)target=&it->second;else g.pull.clear();}if(!target){real bd=std::numeric_limits<real>::max();for(auto* p:survivors){bool ok=g.type=="heart"?p->hp<p->maxHp:g.type=="greenGem"?p->specialCharge<100:true;if(!ok)continue;real d=distanceSq({g.x,g.y},{p->x,p->y});if(d<bd){bd=d;target=p;}}}if(!target)continue;real d2=distanceSq({g.x,g.y},{target->x,target->y});if(!g.pull.empty()||d2<target->pickupRadius*target->pickupRadius){real a=std::atan2(target->y-g.y,target->x-g.x),step=std::min(std::sqrt(d2),(g.pull.empty()?350.0:620.0)*dt);g.x+=std::cos(a)*step;g.y+=std::sin(a)*step;}if(distanceSq({g.x,g.y},{target->x,target->y})<24.0*24.0)collectOne(s,g,*target,rnd);}}

void revive(GameState& s,real dt){for(auto& [_,p]:s.players)p.reviving.clear();for(auto& [id,p]:s.players)if(!p.alive){Player* helper=nullptr;auto hi=s.players.find(p.reviveBy);auto can=[&](Player& q){return q.alive&&q.pendingPowers.empty()&&q.reviving.empty()&&distanceSq({q.x,q.y},{p.x,p.y})<=44.0*44.0;};if(hi!=s.players.end()&&can(hi->second))helper=&hi->second;else for(auto& [_,q]:s.players)if(can(q)){helper=&q;break;}if(!helper){p.reviveProgress=0;p.reviveBy.clear();continue;}if(p.reviveBy!=helper->id)p.reviveProgress=0;p.reviveBy=helper->id;helper->reviving=p.id;int guard=rankOf(*helper,"guardian");p.reviveProgress=std::min(4.0,p.reviveProgress+dt*(1+guard*.5));if(p.reviveProgress>=4){p.alive=true;p.hp=p.maxHp*(.4+guard*.15);p.invulnerableFor=3;p.input={};p.hitCooldown=0;p.pendingPowers.clear();p.reviveProgress=0;p.reviveBy.clear();helper->reviving.clear();++helper->stats.revives;pushEvent(s,"revive",p.x,p.y);}}}
