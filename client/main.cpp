...
struct RGB{Uint8 r,g,b;}; static constexpr RGB PC[CHARACTER_COUNT]={{118,223,255},{255,120,72},{126,225,98},{194,145,255},{115,255,228},{255,215,120}};
...
static void drawView(SDL_Renderer*r,const GameState&s,const Player&cam,SDL_Rect vp){
  ...
  for(const auto&sh:s.shots){auto q=w2s(sh.x,sh.y,cam,vp);circle(r,q.x,q.y,4,PC[std::clamp(sh.color,0,CHARACTER_COUNT-1)]);}
  for(const auto&sh:s.enemyShots){auto q=w2s(sh.x,sh.y,cam,vp);circle(r,q.x,q.y,4,{255,80,80});}
  for(const auto&[_,p]:s.players){auto q=w2s(p.x,p.y,cam,vp);circle(r,q.x,q.y,13,PC[std::clamp(p.color,0,CHARACTER_COUNT-1)]);int w=50;SDL_Rect bg{q.x-w/2,q.y-24,w,5},fg=bg;fg.w=(int)(w*(p.hp/std::max(1.0,p.maxHp)));SDL_SetRenderDrawColor(r,50,50,60,255);SDL_RenderFillRect(r,&bg);SDL_SetRenderDrawColor(r,90,230,120,255);SDL_RenderFillRect(r,&fg);}
}
#ifdef ARCANA_HAS_NET
